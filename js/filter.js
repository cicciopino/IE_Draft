...
const displayResults = (filteredPlayers) => {
  ...
  filteredPlayers.forEach(player => {
    ...
    playerDiv.innerHTML = `
      ...
      <img src="images/${player.Sprite}" alt="${player.EnglishName}">
      <p>Position:
...
const showRandomPlayers = () => {
  ...
  currentPlayerIndex = 0; // Reset dell'indice del giocatore corrente

  // Mostra il primo giocatore
  showNextPlayer();
};

const showNextPlayer = () => {
  if (currentPlayerIndex < randomPlayers.length) {
    const player = randomPlayers[currentPlayerIndex];
    ...
    // Inizializza SortableJS per il giocatore corrente
    new Sortable(playersContainer, {
      group: 'shared', // Gruppo per permettere il trascinamento tra i giocatori
      animation: 150, // Durata dell'animazione in millisecondi
      ghostClass: 'ghost', // Classe per l'elemento fantasma durante il trascinamento
    ...
    currentPlayerIndex++; // Incrementa l'indice del giocatore corrente
  }
};
  ...
  bottomSlots.forEach(slot => {
    new Sortable(slot, {
      ...
      onAdd: function (event) {
        ...
        // Mostra il prossimo giocatore dopo che il precedente è stato trascinato
        showNextPlayer();
      }
    });
  });
...
// Inizializza gli slot al caricamento della pagina
initializeSlots();
